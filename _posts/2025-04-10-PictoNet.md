...
Conviene aclarar una distinción. **PictoNet** es el proyecto hipotético de construir el modelo local: la instanciación propia, el gemelo digital del sistema generativo. [pictos.net](https://pictos.net) es un provotipo — una superficie para explorar y conversar sobre él.

Trabajar con ese provotipo ha hecho visible algo sobre el pipeline. Lo que inicialmente eran tres fases lineales se ha ido desagrupando: cada punto de control resulta ser un nodo con su propia tensión entre lo verbal y lo visual. El pipeline puede ser cualquier grafo con un `inpoint` (texto) y un `outpoint` (SVG). Un *harness*: arnés que sostiene y transmite, cuya topología no está dada de antemano. La *Dual Coding Theory* (Paivio, 1971) — texto entra por el canal verbal, imagen sale por el visual — describe el caso mínimo de ese grafo.

La pregunta práctica que sigue: ¿cuál es el mejor nodo intermedio en términos de precio-calidad? La respuesta provisional es una **descripción de escena en JSON** — intención comunicativa, entidades visuales, relaciones, estilo. Se genera barato, es legible, editable y cabe en el `<metadata>` del SVG. El punto del grafo donde la intervención tiene más palanca.

Una intuición que viene con esto: antes, cada libro era un objeto que envejecía y se reemplazaba. **La pantalla permanece; el contenido es el reciclable.** La pantalla es el anti-material.
...
| **2. Extracción de entidades visuales** | NER visual             | Identifica entidades representables gráficamente (acciones, objetos, lugares)      | spaCy NER(([spaCy NER](https://spacy.io/models).)), XLM-RoBERTa(([XLM-RoBERTa](https://huggingface.co/xlm-roberta-base).))       |
| **3. Blending conceptual**     | Composición semántica | Fusiona elementos en una imagen coherente (ej. "calentar comida en microondas")    | ConceptNet(([ConceptNet](https://conceptnet.io/).)), SBERT(([SBERT](https://huggingface.co/sentence-transformers/paraphrase-multilingual-mpnet-base-v2).)) |
| **4. Selección de foco y layout** | Estructura discursiva  | Establece jerarquía entre elementos visuales y su disposición                      | LayoutLM(([LayoutLM](https://huggingface.co/microsoft/layoutlm-base-uncased).)), FLAN-T5(([FLAN-T5](https://huggingface.co/google/flan-t5-base).)) |
| **5. Atributos gráficos**      | Mapeo semiótico       | Asocia propiedades semánticas con atributos visuales (color, animación, forma)     | Sketch-RNN(([Sketch-RNN](https://github.com/magenta/magenta/tree/main/magenta/models/sketch_rnn).)), CLIP(([CLIP](https://huggingface.co/openai/clip-vit-base-patch32).)) |
...
La ambición de esta capa no es sólo tecnológica. Apunta a imaginar una **nueva economía de las inteligencias artificiales**: una economía basada en colaboración, transparencia y responsabilidad compartida. Frente al modelo tecnofeudal((Varoufakis, Y. (2023). *Technofeudalism: What Killed Capitalism*. The Bodley Head.)), donde los usuarios entregan trabajo cognitivo a cambio de servicios opacos, PictoNet propone una economía simbólica descentralizada, donde cada quien conserva el control sobre su lenguaje, su historia y sus contribuciones.
...
Este enfoque se opone a las lógicas de extracción de datos y control unidireccional que caracterizan a buena parte de las inteligencias artificiales contemporáneas. Frente a esa deriva tecnofeudal((Zuboff, S. (2019). *The Age of Surveillance Capitalism*. PublicAffairs; Varoufakis, Y. (2023). *Technofeudalism: What Killed Capitalism*. The Bodley Head.)), **MediaFranca** plantea un ecosistema convivencial y distribuido, donde los usuarios no son datos, sino autores.
