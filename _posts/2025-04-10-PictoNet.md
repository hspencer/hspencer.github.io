...
Desde que escribí este texto he estado trabajando con un provotipo: [pictos.net](https://pictos.net). Conviene aclarar la diferencia entre ambas cosas, porque no son lo mismo.

**PictoNet** es el proyecto hipotético de construir el modelo local — la instanciación propia, el gemelo digital del sistema generativo. Una infraestructura que no depende de servicios externos y que puede ser controlada, ajustada y apropiada por quienes la usan.

**pictos.net** es otra cosa: un provotipo. No una implementación del modelo, sino una superficie para conversar sobre él. Y lo que esa superficie ha permitido explorar es la naturaleza de los *puntos de control* — los nodos que articulan el pipeline.

Inicialmente, el pipeline generativo se pensó como una secuencia lineal: fases 1, 2, 3. Pero al trabajar con el provotipo, cada punto de control se fue desagrupando. Lo que parecía un paso fue revelando ser un nodo con su propia tensión interna — una *gradiente*, una polaridad entre lo visual y lo verbal. El punto de control no es una caja negra anónima: es un momento de razonamiento con su propia textura semiótica, su propio registro.

De este modo, el pipeline dejó de ser lineal para convertirse en una trama. Un grafo. Un *harness* — en el sentido de arnés, estructura de contención y transmisión — donde la generalización sigue siendo la idea que permanece, pero la forma de organizarla se ha vuelto más compleja y más honesta.

Hay algo más, y quizás es lo más importante: el pipeline como grafo revela una diferencia ontológica con los medios anteriores. Antes, cada libro era una presencia física: un objeto que ocupaba espacio, que se reemplazaba, que envejecía. La pantalla opera de otro modo. **La pantalla permanece; el contenido es el reciclable.** No se tiran pantallas: se actualizan. No se coleccionan interfaces: se habitan. En ese sentido, la pantalla es el *anti-material* — no porque sea inmaterial, sino porque su materialidad es de otro orden: persiste mientras todo lo que pasa por ella es efímero, intercambiable, regenerable.

Esta intuición cambia cómo entiendo PictoNet. Ya no es solo un motor de traducción texto-imagen. Es una infraestructura de *harness* semiótico — un arnés que sostiene flujos de sentido entre nodos, donde cada nodo tiene su gradiente, su polaridad, su momento de razonamiento visual-verbal. Y donde la pantalla — el sistema — permanece como el soporte que hace posible la renovación continua del contenido.

Una forma de leer este grafo es desde la *Dual Coding Theory* (Paivio, 1971): el sistema verbal y el sistema imaginístico como dos canales separados que se referencian mutuamente, con un nodo de cruce entre ellos. Eso haría el pipeline `texto → [verbal] ⇌ [imagen] → SVG` — un grafo mínimo. Pero es una simplificación: asume que el cruce verbal↔visual ocurre en un solo punto, cuando en realidad cada nodo del harness tiene su propia tensión entre lo verbal y lo visual. El grafo generalizado es más honesto: cualquier estructura con un `inpoint` (texto) y un `outpoint` (SVG) es válida. La forma del grafo no está dada de antemano.

La pregunta práctica que sigue es: ¿cuál es el mejor *nodo intermedio* en términos de precio-calidad? Mi hipótesis actual es que ese nodo es una **descripción de escena estructurada** — un objeto JSON pequeño que especifica la intención comunicativa, las entidades visuales, sus relaciones espaciales y los parámetros de estilo. Es barato de generar (cualquier LLM compacto lo produce bien), es inspeccionable (texto legible, no un embedding opaco), es editable (el profesional puede intervenirlo directamente) y es trazable (cabe íntegro en el `<metadata>` del SVG). Es, en ese sentido, el *handle* más útil del pipeline: el punto donde la intervención tiene más palanca con el menor costo técnico.
